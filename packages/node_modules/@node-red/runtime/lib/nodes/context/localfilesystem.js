...
LocalFileSystem.prototype.incr = function(scope, key, amount = 1, callback) {
    var self = this;
    var storagePath = getStoragePath(this.storageBaseDir ,scope);
    if (this.cache) {
        var result = this.cache.incr(scope,key,amount,callback);
        this.pendingWrites[scope] = true;
        if (!this._pendingWriteTimeout) {
            this._pendingWriteTimeout = setTimeout(function() {
                self.writePromise = self.writePromise.then(function(){
        ...
        self.writePromise = self.writePromise.then(function() { return loadFile(storagePath + ".json"); }).then(function(data){
            var obj = data ? JSON.parse(data) : {};
            var value = util.getObjectProperty(obj, key);
            if(value === undefined){
                value = 0;
            }
            if(typeof value === "number"){
                value = value + amount;
                util.setObjectProperty(obj,key,value);
            }else{
            ...
            return fs.outputFile(storagePath + ".json", stringifiedContext.json, "utf8").then(function(){
                if(typeof callback === "function"){
                    callback(null, value);
                }
            });
